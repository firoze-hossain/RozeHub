...
<div class="release-form-section"><div class="release-form-section-head"><span>PACKAGE</span><strong>Upload once, distribute everywhere</strong></div>
<label>Installer / package <span class="hint">Large files are uploaded in 4 MB chunks, so the complete installer does not need to fit inside one PHP POST request.</span><input class="file-drop" type="file" name="package" {{ $mode==='create'?'required':'' }}></label>
@if($mode==='edit' && $release->file_name)<div class="current-file release-current-file"><strong>Current package:</strong> {{ $release->file_name }} · {{ $release->file_size ? number_format($release->file_size/1048576,1).' MB' : 'size unavailable' }} @if($release->sha256)<br><strong>SHA-256:</strong> <code>{{ $release->sha256 }}</code>@endif<br><small>Stored at <code>{{ $release->file_path }}</code></small></div>@endif
