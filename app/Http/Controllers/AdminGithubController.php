...
class AdminGithubController extends Controller {
 ...
 public function sync(SoftwareProject $project,GithubService $github,AnalyticsService $analytics){try{$github->sync($project);$analytics->track('github_sync',$project->id,$project,[],request());return back()->with('success','GitHub repository synchronized successfully.');}catch(\Throwable $e){return back()->with('error',$e->getMessage());}}
